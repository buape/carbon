...
Modals are popup forms that can be used to collect user input. They are created by extending the [`Modal`](/api/carbon/modal) class, and adding a `title` and `components` property. The components must be [`Label`](/api/carbon/label) classes that each contain a single [`TextInput`](/api/carbon/textinput) or [`StringSelectMenu`](/api/carbon/stringselectmenu) component.
