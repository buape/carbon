...
The [`Poll`](/api/carbon/poll) class represents a poll that has been received from Discord, typically accessed through a `Message` object. This class is designed for reading and interacting with existing polls, not for creating new ones.
