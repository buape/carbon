...
Extend the `ClientManager` class and override methods like `getClient`, `getClients`, and `getClientIds` as needed. The ClientManager provides a `setupClient()` helper that you call with credentials to create a configured client:
...
`baseUrl` and `deploySecret` are configured on `ClientManager` itself.
...
// Get all clients
const allClients = manager.getClients()
...
If you're managing lots of apps, startup behavior matters.

You can set startup policy directly on `ClientManager`:
...
- `concurrency`: how many clients initialize in parallel
- `failFast`: stop scheduling new startup work after first failure

You can also explicitly wait for startup completion:

```typescript
const startup = await manager.whenReady()
console.log(startup)
```

That result includes totals plus per-client success/error outcomes, which is handy for readiness checks and startup logs.
