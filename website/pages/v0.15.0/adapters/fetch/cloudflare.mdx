...
<Step title="Create a Entry Point File">
...
<Step title="Add a Wrangler Configuration">
...
## Gateway Events on Workers

Need Gateway events (like `MESSAGE_CREATE`) on Workers? Use `CloudflareGatewayPlugin` with `CloudflareGatewayDurableObject`.

`create-carbon` can scaffold this automatically when you choose Cloudflare + Gateway + Durable Objects.

At a high level:

1. Add `CloudflareGatewayPlugin` to `Client` plugins.
2. Export `CloudflareGatewayDurableObject` from your Worker entry module.
3. Add `CARBON_GATEWAY` Durable Object binding + migration in `wrangler.jsonc`.
4. Pass `{ env }` through the fetch adapter context:
...
Workers + Durable Objects are elastic, but this setup is not infinite-scale by default:

- Carbon's default Cloudflare gateway setup targets one Durable Object instance.
- That instance is single-threaded.
- Gateway events are forwarded from the Durable Object back to your Worker over HTTP.

For detailed throughput and pricing math (including the 10 events/sec example), see [Gateway plugin docs](/plugins/gateway#cloudflare-scaling-and-pricing-notes).
