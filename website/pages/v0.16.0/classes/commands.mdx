...
To create a command, you need to create a class that extends the [`Command`](/api/carbon/command) class. This class will handle the command and its functionality.
...
- `interaction.targetMessage` returns a fully constructed [`Message`](/api/carbon/message) for message-context commands (equivalent to the resolved message in the raw data).
- `interaction.targetUser` returns the resolved [`User`](/api/carbon/user) for user-context commands.

These helpers fall back to `null` if the command isn’t of the matching type.

### Mentioning Commands

Every command tracks its Discord ID after a deploy, and exposes an `await command.getMention(client)` helper that returns the `</name:id>` string. Example:

```ts
const mention = await this.getMention(client)
await interaction.reply(`Use ${mention} to learn more.`)
```

If the ID isn’t cached yet, Carbon fetches the current commands once via `client.getDiscordCommands()` and updates the in-memory commands so future calls don’t re-fetch.
...
The `middlewares` property lets you attach per-command middleware hooks:

- `before(context)` runs before command execution.
- `after(context)` runs after execution with status and timing details.

For global middleware shared across all commands, use `ClientOptions.commandMiddlewares`.
