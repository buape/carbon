...
Text Inputs are blank text fields used solely within [Modals](/api/carbon/modal) to collect user input. They are created by extending the `TextInput` class, and adding a `label` and `placeholder` property.
