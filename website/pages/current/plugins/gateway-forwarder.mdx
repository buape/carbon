...
You can control forward delivery behavior directly in plugin config.

Useful when your endpoint is slow, flaky, or under load.
...
- `timeoutMs`: max time for a single forward attempt
- `maxRetries`: retry attempt limit
- `baseBackoffMs` / `maxBackoffMs` / `jitterRatio`: retry timing behavior
- `maxInFlight`: parallel delivery cap
- `maxQueueSize`: max buffered deliveries before dropping

Forwarded requests also include:

- `X-Carbon-Forwarder-Event-Id`
- `X-Carbon-Forwarder-Attempt`

Those make receiver-side dedupe and debugging much easier.

## Delivery Metrics

Need delivery stats at runtime?

```typescript
const forwarder = client.getPlugin<GatewayForwarderPlugin>("gateway-forwarder")
const metrics = forwarder?.getDeliveryMetrics()
```

Includes queue depth, in-flight count, sent/retried/failed/dropped counters, and failure reason counts.
