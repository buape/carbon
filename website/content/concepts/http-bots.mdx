...
However, there are some downsides to this approach for some applications, mainly that the API Gateway requires a constant connection. Pure stateless serverless runtimes can't hold this connection directly. On platforms like Vercel (or Workers without Durable Objects), this is a problem.
