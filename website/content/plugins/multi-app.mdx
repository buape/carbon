...
The ApplicationManager eliminates all of this by acting as a smart proxy that routes incoming requests to the appropriate bot based on the URL path.

## How It Works

The ApplicationManager creates multiple `Client` instances internally - one for each bot you want to host. When Discord sends a request to your server, the ApplicationManager looks at the URL path to determine which bot should handle it:
...
Here's how to set up the ApplicationManager with multiple bots:

```typescript title="src/index.ts"
import { ApplicationManager } from "@buape/carbon"
...
For advanced use cases like bot-as-a-service platforms, you may need to load bot credentials dynamically from a database instead of hardcoding them. The ApplicationManager is designed to support this through class extension and method overriding.

### Extending ApplicationManager

Extend the `ApplicationManager` class and override the `getClient`, `getAllClients`, and `getClientIds` methods. The ApplicationManager provides a `setupClient()` helper that you call with credentials to create a configured client:

```typescript title="src/DatabaseApplicationManager.ts"
import {
	type ApplicationCredentials,
	ApplicationManager,
	type ApplicationManagerOptions
} from "@buape/carbon"

export class DatabaseApplicationManager extends ApplicationManager {
	...
	constructor(
		options: Omit<ApplicationManagerOptions, "applications">,
...
import { DatabaseApplicationManager } from "./DatabaseApplicationManager"
import { db } from "./database"

const manager = new DatabaseApplicationManager(
	{
		sharedOptions: {
			baseUrl: process.env.BASE_URL,
			deploySecret: process.env.DEPLOY_SECRET
...
The ApplicationManager works with any Carbon adapter. Here's an example with the Node.js adapter:
...
Each bot in your ApplicationManager is set up to be a mirror of the other bots. When registering commands and listeners, pass them through the `sharedOptions`:
...
The ApplicationManager provides a global deploy endpoint at `/deploy` that deploys commands for all bots at once:
...
The ApplicationManager provides several methods to access the underlying clients:
...
The ApplicationManager includes built-in validation:
...
**Request Routing:**
If Discord sends a request to an unknown client ID, the ApplicationManager returns a 404:
...
The ApplicationManager is perfect for:
