...
Embeds are used to display rich content in messages. They are created by extending the [`Embed`](/carbon/api/index/classes/Embed) class, and adding a `title`, `description`, and `url` property.
...
Here we have created an embed with a title, description, and URL. You can use this embed in a message by using the [`Embed`](/carbon/api/index/classes/Embed) class.
