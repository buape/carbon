...
Modals are popup forms that can be used to collect user input. They are created by extending the [`Modal`](/carbon/api/index/classes/Modal) class, and adding a `title` and `components` property. All the components must be [`TextInput`](/carbon/api/index/classes/TextInput) classes.
